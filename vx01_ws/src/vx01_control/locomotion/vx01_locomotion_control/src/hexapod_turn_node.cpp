...
void HexapodTurnNode::declareParameters()
{
    ...
    declare_parameter("turn_direction",    1);
...
void HexapodTurnNode::loadParameters()
{
...
CallbackReturn HexapodTurnNode::on_configure(const rclcpp_lifecycle::State&)
{
...
CallbackReturn HexapodTurnNode::on_cleanup(const rclcpp_lifecycle::State&)
{
    locomotion_.reset();
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (locomotion_->isSwingPhase(i)) {
            ...
            for (int k = 1; k <= N; ++k) {
                ...
                double th1, th2, th3;
                sampleTurnSwingAt(i, gt, th1, th2, th3);
                if (k == N) {
                    last_sent_angles_[i*3+0] = th1;
                    last_sent_angles_[i*3+1] = th2;
                    last_sent_angles_[i*3+2] = th3;
    ...
    double& theta1, double& theta2, double& theta3)
{
    ...
    vx01_hexapod_locomotion::kinematics::InverseKinematics ik(L1_, L2_, L3_);
    double th1=0.0, th2=0.0, th3=0.0;
    if (!ik.compute(home_x_, foot_y, foot_z, th1, th2, th3)) {
    ...
    double& theta1, double& theta2, double& theta3)
{
    ...
    vx01_hexapod_locomotion::kinematics::InverseKinematics ik(L1_, L2_, L3_);
    double th1=0.0, th2=0.0, th3=0.0;
    if (!ik.compute(home_x_, foot_y, home_z_, th1, th2, th3)) {
        th1 = last_sent_angles_[leg_index*3+0];
        th2 = last_sent_angles_[leg_index*3+1];
        th3 = last_sent_angles_[leg_index*3+2];
    }
