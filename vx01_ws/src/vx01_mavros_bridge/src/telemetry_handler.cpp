...
namespace vx01_mavros_bridge {
    ...
    TelemetryHandler::TelemetryHandler(rclcpp::Node::SharedPtr node)
        ...
        // Subscribe to state
        state_sub_ = node_->create_subscription
        ...
        // Subscribe to GPS
        gps_sub_ = node_->create_subscription
        ...
        local_pos_sub_ = node_->create_subscription
        ...
        altitude_sub_ = node_->create_subscription
        ...
        RCLCPP_INFO(node_->get_logger(),
            "TelemetryHandler initialized");
    }

    bool TelemetryHandler::isConnected() const {
    ...
    std::string TelemetryHandler::getFlightMode() const {
        return current_state_.mode;
    }

    double TelemetryHandler::getBatteryVoltage() const {
        return current_battery_.voltage;
    }

    double TelemetryHandler::getBatteryPercentage() const {
        return current_battery_.percentage * 100.0;
    }

    bool TelemetryHandler::isBatteryLow() const {
        // Low battery threshold: 20%
        return getBatteryPercentage() < 20.0;
    }

    double TelemetryHandler::getLatitude() const {
        return current_gps_.latitude;
    }

    double TelemetryHandler::getLongitude() const {
        return current_gps_.longitude;
    }

    double TelemetryHandler::getGpsAltitude() const {
        return current_gps_.altitude;
    }

    bool TelemetryHandler::isGpsFixed() const {
        return current_gps_.status.status >= 0;
    }

    double TelemetryHandler::getLocalX() const {
        return current_local_pos_.pose.position.x;
    }

    double TelemetryHandler::getLocalY() const {
        return current_local_pos_.pose.position.y;
    }

    double TelemetryHandler::getLocalZ() const {
        return current_local_pos_.pose.position.z;
    }

    double TelemetryHandler::getAltitude() const {
        return current_altitude_;
    }

    bool TelemetryHandler::isStateReceived() const {
        return state_received_;
    }

    bool TelemetryHandler::isBatteryReceived() const {
        return battery_received_;
    }

    bool TelemetryHandler::isGpsReceived() const {
        return gps_received_;
    }

    bool TelemetryHandler::isPositionReceived() const {
    ...
    void TelemetryHandler::printTelemetry() const {
        ...
        RCLCPP_INFO(node_->get_logger(),
            "Armed     : %s", isArmed() ? "YES" : "NO");
        RCLCPP_INFO(node_->get_logger(),
            "Mode      : %s", getFlightMode().c_str());
        RCLCPP_INFO(node_->get_logger(),
            "Battery   : %.1f%% (%.2fV)",
            getBatteryPercentage(), getBatteryVoltage());
        RCLCPP_INFO(node_->get_logger(),
            "Altitude  : %.2f m", getAltitude());
        RCLCPP_INFO(node_->get_logger(),
